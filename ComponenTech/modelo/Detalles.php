...
class Detalles {
	...
	public function getDetallesUsuario($u){
		$sql=" SELECT idDetalles,cantidad,totalCantidad,USUARIO_documento,idFactura,FACTURA_idFactura,productoNombre,precio,prodImg,idProducto,PRODUCTO_idProducto,fecha from detalles,producto,factura where USUARIO_documento=$u and (FACTURA_idFactura=idFactura and PRODUCTO_idProducto=idProducto) order by idDetalles desc";
		$cn = conectar();
		$res = $cn->query($sql);
		$cn->close();
		return $res;
